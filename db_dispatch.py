...
#       return(item_list)
    ...
    def db_connect(self):
        """Connect to the Dispatch Log database"""

        self.db_pathname = self.cmn.stns.get_pathname_dispatch_db()
        self.db_conn_str = (r'DRIVER={Microsoft Access Driver '
            + r'(*.mdb, *.accdb)}; DBQ=' + self.db_pathname
            + '; Mode=Read;')

        try:
            logging.info(f"Attempting connection to {self.db_pathname}")
            import pyodbc
    ...
    def db_cursor(self):
        ...
        if self.conn:
            try:
                ...
                logging.info(f"Error creating database cursor: {e}")
                self.curs = None
    ...
    def db_table_list(self):
        ...
        if self.conn:
            self.db_cursor()
            if self.curs:
                for i in self.curs.tables(tableType='TABLE'):
                    items.append(i.table_name)
                self.curs.close()
                self.conn.close()
        self.conn = None
        return items
    ...
    if codes:
        ...
        for i in codes:
            ...
            if not i[0]:
                act = " XX "
            code1 = str(i[1])
            code2 = code1[:2] + ":" + code1[2:]
            print(f"{act:4} {code2:5} {i[2]}")
    print()
